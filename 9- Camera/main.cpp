...
const GLchar* fragmentSource =
    ...
    "void main() {"
    "   outColor = vec4(Color, 1.0) * mix(texture(texKitten, Texcoord), "
...
int main() {
    ...
    while (!glfwWindowShouldClose(window)) {
            ...
            glfwSetWindowShouldClose(window, GL_TRUE);
        {
			...
			auto direction = glm::normalize(cameraPosition * glm::vec3(-1, -1, -1));
			auto right     = glm::normalize(glm::cross(direction, glm::vec3(0, 0, 1)));
			//auto up        = glm::normalize(glm::cross(direction, right));

			auto rotateX = static_cast<float>(lastPose.x - x) * -0.01f;
			auto rotateY = static_cast<float>(lastPose.y - y) * -0.01f;
			
			auto r = glm::rotate(glm::mat4(), rotateX, glm::vec3(0, 0, 1));
			r = glm::rotate(r, rotateY, right);
            auto p = r * glm::vec4(cameraPosition, 1);
